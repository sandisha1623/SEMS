...
$routes->group(
    ...
    static function ($routes) {
        ...
        $routes->get('(:segment)/edit',    'ExamSessionController::edit/$1');
        $routes->post('(:segment)/update', 'ExamSessionController::update/$1');
        $routes->post('(:segment)/delete', 'ExamSessionController::delete/$1');

        // Nested: participants management per session
        $routes->get(
            '(:segment)/participants',
            'ExamParticipantController::index/$1'
        );
        $routes->post(
            '(:segment)/participants/enroll',
            'ExamParticipantController::enroll/$1'
        );
        $routes->post(
            '(:segment)/participants/(:segment)/status',
            'ExamParticipantController::updateStatus/$1/$2'
        );
        $routes->post(
            '(:segment)/participants/(:segment)/delete',
            'ExamParticipantController::delete/$1/$2'
        );
    }
);
...
$routes->group(
    ...
    static function ($routes) {
        ...
        $routes->get('(:segment)/edit',    'DepartmentController::edit/$1');
        $routes->post('(:segment)/update', 'DepartmentController::update/$1');
        $routes->post('(:segment)/delete', 'DepartmentController::delete/$1');
    }
);

$routes->group(
    'users',
    ['filter' => 'web-permission:users.manage'],
    static function ($routes) {
...
$routes->group('api', static function ($routes) {
    ...
    $routes->get('jwt-protected', 'Api\TestController::jwtProtected', ['filter' => 'jwt']);
    $routes->post('publish-event', 'Api\TestController::publishEvent');

    $routes->get('jwt-login', 'Api\TestController::jwtLogin');
    $routes->post('jwt-logout', 'Api\TestController::jwtLogout', ['filter' => 'jwt']);
    $routes->post('refresh-token', 'Api\TestController::refreshToken');

    $routes->get('protected', 'Api\TestController::protected', ['filter' => 'auth']);
    $routes->get('admin', 'Api\TestController::admin', ['filter' => 'role:super-admin,admin']);
    $routes->get('payment', 'Api\TestController::payment', ['filter' => 'permission:finance.payment.verify']);

    $routes->get('throttle-test', 'Api\TestController::public', ['filter' => 'throttle:5,60']);
    ...
    $routes->get(
        'exam-sessions/(:segment)/participants/data',
        'Api\ExamParticipantApiController::data/$1',
        ['filter' => 'web-permission:exam.manage']
    );
    ...
    $routes->get(
        'users/data',
        'Api\UserApiController::data',
        ['filter' => 'web-permission:users.manage']
    );

    // Search endpoint untuk modal enrollment
    $routes->get(
        'users/searchable-students',
        'Api\UserApiController::searchableStudents',
        ['filter' => 'web-permission:exam.manage']
    );
});
